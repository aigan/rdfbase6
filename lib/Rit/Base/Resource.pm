...
Calls L</log_stats> with C<views>.

=cut

sub log_view   { shift->log_stat('views') }
...
Calls L</log_stats> with C<searches>.

=cut

sub log_search { shift->log_stat('searches') }
...
Calls L</log_stats> with C<email>.

=cut

sub log_email  { shift->log_stat('emails') }
...
Calls L</log_stats> with C<clicks>.

=cut

sub log_click  { shift->log_stat('clicks') }
...
Calls L</log_stats> with C<bviews>.

=cut

sub log_bview  { shift->log_stat('bviews') }
...
Calls L</log_stats> with C<bclicks>.

=cut

sub log_bclick { shift->log_stat('bclicks') }
...
Set up stats to be changed at a later time. Wait with these to avoid
deadlocks.
...
sub log_stat
{
    ...
    # Handle spelling
    $type =~ s/^(b?)clicks$/${1}klicks/;

#    debug "Logging $node->{id} stats $type";

    $Rit::Base::Cache::stats_change{$type} ||= [];
    push @{$Rit::Base::Cache::stats_change{$type}}, $node;
    return '';
}
